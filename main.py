...
def detect_url():
    v = os.getenv("WEBAPP_URL","") or os.getenv("RENDER_EXTERNAL_URL","")
    if v: return v
    v = os.getenv("RENDER_EXTERNAL_HOSTNAME","")
    if v: return f"https://{v}"
    v = os.getenv("RAILWAY_PUBLIC_DOMAIN","")
    if v: return f"https://{v}"
    try:
...
def main():
    ...
    # Always use polling — Flask handles the web routes, bot uses Telegram's servers
    log.info(f"🤖 Bot polling... | 🌐 {PUBLIC_URL}/admin | 👤 {PUBLIC_URL}/panel")
